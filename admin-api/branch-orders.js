...
var branchOrderCalls = {
  ...
  removeOneItem: (req, res) => {
    try {
      ...
      PastryOrder.findById(bodyData.order, (err, orders) => {
        if (err) {
          res.status(400).json({
            message: err.message,
          });
          ...
          if (orders.pastry && orders.pastry.length > 0) {
            ...
            if (tempPastries.length === 0) {
              PastryOrder.deleteOne({ _id: bodyData.order }, (err, success) => {
                if (err) {
                  res.status(400).json({
                    message: err.message,
                  });
  ...
  getAllBillOrdersByBranch: (req, res, next) => {
    var branch = req.params.branch;
    var sQuery = {
      created_by: new Mongoose.Types.ObjectId(branch),
      bill_status: "1",
    };
    var addQuery = {};
    var productQ = {};
    ...
    if (req.query.liveCategory && req.query.liveCategory.length > 0) {
      // sQuery.category = new Mongoose.Types.ObjectId(req.query.liveCategory);
      const catArr = [];

      req.query.liveCategory.forEach((item) => {
        catArr.push(new Mongoose.Types.ObjectId(item));
      });
      catQ = {
    ...
    if (req.query.liveOrderStatus) {
      addQuery = { "pastry.status": req.query.liveOrderStatus };
    }
    if (req.query.liveProducts && req.query.liveProducts.length > 0) {
      const productArr = [];

      req.query.liveProducts.forEach((item) => {
        productArr.push(new Mongoose.Types.ObjectId(item));
      });
      productQ = {
        "pastry._id": { $in: productArr },
    ...
    cond = [
      ...
      { $match: addQuery },
      { $match: productQ }, // filter only product
      {
        $lookup: {
          from: "productcategories",
          localField: "category",
    ...
    PastryOrder.aggregate(cond).exec((err, items) => {
      if (err) {
        ...
        if (items[0] && items[0]._id) {
          PastryPayment.findOne({ bill_no: items[0]._id }, (error, payment) => {
            if (err) {
              res.status(400).json({
  ...
  updateQty: (req, res, next) => {
    try {
      ...
      // order total
      PastryOrder.findById(bodyData.orderId, (err, result) => {
        if (err) {
          res.status(400).json({
            message: err.message,
          });
          ...
          tempPastry.forEach((element) => {
            if (element._id == bodyData.pastry) {
              totalProductPrice =
                bodyData.sendQty * element.price - element.sendTotal;
                ...
                PastryPayment.findOne(
                  ...
                  (fail, payments) => {
                    if (fail) {
                      ...
                      const newPrice =
                        parseFloat(payments.price) +
                        parseFloat(totalProductPrice);
                      const oldBalance =
                        parseFloat(payments.balance) +
                        parseFloat(totalProductPrice);

                      PastryPayment.updateOne(
                        { bill_no: payments.bill_no },
                        {
                          $set: {
                            price: newPrice,
                            balance: oldBalance,
  ...
  closeBill: (req, res, next) => {
    try {
      var bodyData = req.body;
      var sQuery = {
        created_by: new Mongoose.Types.ObjectId(bodyData.branch),
        bill_no: bodyData.billno,
      };

      //pastry status, status
      PastryOrder.find(sQuery, (err, result) => {
        if (err) {
          res.status(400).json({
            message: error.message,
          });
          ...
          result.forEach((orders) => {
            const pastry = orders.pastry;
            pastry.forEach((element) => {
              element.status = "2";
            });
            PastryOrder.updateOne(
              { _id: orders._id },
              { $set: { bill_status: "2", pastry: pastry } },
              (error, modified) => {
                if (err) {
                  console.log(err);
                }
  ...
  updateBalancePayment: (req, res, next) => {
    try {
      ...
      PastryPayment.findOne({ bill_no: billno }, (err, result) => {
        if (err) {
          res.status(400).json({
            message: err.message,
          });
      ...
      PastryOrder.aggregate(cond).exec((err, items) => {
        if (err) {
          ...
          items.map((item, key) => {
            ...
            ws.cell(cellStart, 7).string(
              item.employees ? item.employees.firstname : "N/A"
            );
